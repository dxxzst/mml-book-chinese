...
_备注 _ 。在机器学习文献中，线性与仿射之间的区别有时并不严格分明，因此我们经常会看到将 _ 仿射空间（affine spaces）_ / _ 仿射映射（affine mappings）_ 称为 _ 线性空间（linear spaces）_ / _ 线性映射（linear mappings）_ 的情况。♦
...
称为 $ V $ 的 _仿射子空间（affine subspace）_ 或 _ 线性流形（linear manifold）_ 。$ U $ 称为 _方向（direction）_ 或 _ 方向空间（direction space）_ ，$ \boldsymbol x_0 $ 称为 _支撑点（support point）_ 。在第 12 章中，我们将此类子空间称为 _ 超平面（hyperplane）_ 。
