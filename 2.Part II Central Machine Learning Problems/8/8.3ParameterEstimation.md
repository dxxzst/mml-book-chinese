...
我们可以考虑针对其他预测任务的其他模型，例如二分类或计数数据建模（McCullagh and Nelder, 1989）。对此的另一种视角是考虑来自指数族的似然函数（6.6 节）。这类在参数和数据之间具有线性依赖关系、并具有潜在非线性变换 $\phi$（称为 _连接函数（link function）_）的模型被称为 _ 广义线性模型（generalized linear models）_ （Agresti, 2002, chapter 4）。
