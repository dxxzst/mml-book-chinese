...
_备注（标准正交基下的正交投影）_ 。简要回顾 3.8 节的正交投影理论：若 $(\boldsymbol{b}_1, \ldots, \boldsymbol{b}_D)$ 为 $\mathbb{R}^D$ 的标准正交基，则向由单个基向量 $\boldsymbol{b}_j$ 张成的一维子空间的正交投影为：
...
_备注（低秩矩阵近似）_ 。向主子空间投影的投影矩阵由下式给出：
