...
令该偏导数为 $\boldsymbol{0}$，在两边左乘并右乘 $\boldsymbol{\Sigma}_k$，即得定理 11.2 的更新公式：
...
>
> $$
> \sigma_1^2: 1 \to 0.14 , \quad \sigma_2^2: 0.2 \to 0.44 , \quad \sigma_3^2: 3 \to 1.53 .
> \tag{11.39}
> $$
>
...
$$
\mathcal{L}_{\pi} = \mathcal{L} + \lambda \left( \sum_{k=1}^K \pi_k - 1 \right) = \sum_{n=1}^N \log \left( \sum_{k=1}^K \pi_k \mathcal{N}\left(\boldsymbol{x}_n \mid \boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k\right) \right) + \lambda \left( \sum_{k=1}^K \pi_k - 1 \right) .
\tag{11.43}
$$
...
$$
\frac{\partial \mathcal{L}_{\pi}}{\partial \pi_k} = \sum_{n=1}^N \frac{\mathcal{N}\left(\boldsymbol{x}_n \mid \boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k\right)}{\sum_{j=1}^K \pi_j \mathcal{N}\left(\boldsymbol{x}_n \mid \boldsymbol{\mu}_j, \boldsymbol{\Sigma}_j\right)} + \lambda = \frac{1}{\pi_k}\sum_{n=1}^N r_{nk} + \lambda = \frac{N_k}{\pi_k} + \lambda .
\tag{11.44}
$$

令该偏导数为 0，得到 $\pi_k = -\frac{N_k}{\lambda}$。由于 $\sum_{k=1}^K \pi_k = 1$，对所有分量求和：
...
>
> $$
> \pi_1: 0.33 \to 0.29 , \quad \pi_2: 0.33 \to 0.29 , \quad \pi_3: 0.33 \to 0.42 .
> \tag{11.50}
> $$
>
